...
## The image finally reaches the GPU at 16 bits

- LibRaw has always decoded at 16 bits per channel (`outputBps: 16`), but the renderer uploaded an **8-bit** texture — `data[i] >> 8` — throwing half the decoder's precision away *before any adjustment ran*. Every bit of colour science in the shader was operating on JPEG-grade data
- That loss landed exactly where this editor is meant to be strong. The shader's first move on a sampled pixel is `srgbToLinear()`, which **expands** dark values, so coarse 8-bit steps become banding the moment shadows are lifted; the highlight reconstruction spliced in at 16 bits by the decoder was quantised away before Highlights could pull it back; and AgX opens with `log2()`, which quantises hardest in the shadows where its toe lives
- Now uploaded as **`RGBA16F`**. Half-float suits image data because its resolution scales with magnitude — shadows, where banding shows first, get far finer steps than a uniform integer format gives them. It's filterable in core WebGL2, so the sharpen and dust taps still sample with `LINEAR`, and **no adjustment maths changed at all**: the shader still receives 0..1 values, just far more finely resolved ones. `toRgba8()` remains as a fallback if a driver rejects the format (this renderer has been bitten by a mobile driver before)
- Conversion is a precomputed uint16→half lookup table (128 KB, built once) and the upload is banded 256 rows at a time, so a 24 MP export never allocates the ~200 MB staging array a single-shot conversion would need — measured at 128 ms for the preview and 527 ms for a full-res export, against a ~13 s decode
- **Verified by A/B against the old path on identical data**, by sabotaging the format check to force the 8-bit fallback and re-rendering the same frame at +4 EV: **17.2% of all colour channels changed, rising to 83.0% of channels in shadow pixels** (max difference 63 levels). The shadow concentration is precisely the predicted signature. A whole-image histogram showed no difference on the ISO 10000 test frame — sensor noise dithers quantisation away — which is itself the useful caveat: the win shows on clean, low-ISO files with smooth gradients, not on noisy ones

## Auto Levels removed

- Deleted the Auto Levels button and `lib/autoLevels.ts` (207 lines). It worked by keeping a **hand-ported copy of the shader's tone maths in JS** so it could predict where the sliders should land — which meant every change to the tone pipeline had to be mirrored in a second place or the button silently started giving bad results. It had already drifted once and solved Contrast to 95
- Removed because that tax fell on exactly the colour-science and tone-mapping work this project most wants to do: a second copy of the maths that quietly rots every time the first copy improves. The `Auto` panel remains for Dust Removal. Recoverable from git history if it's ever wanted; the note in CLAUDE.md records that any future auto should read back the real pipeline rather than re-deriving it
- `computeImageHistogram()` went with it — it existed only to feed the solver
